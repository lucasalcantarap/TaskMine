...
import { Task, TimeOfDay, TaskStatus, UserProfile, Reward, ServerMessage } from '../types';
...
const ChildDashboard: React.FC<ChildDashboardProps> = ({ tasks, profile, rewards, messages = [], onCompleteTask, onBuyReward, onUpdateProfile }) => {
    ...
    const nextLevelXp = getXpThreshold(profile.level);
    ...
    return (
        <div className="w-full max-w-6xl mx-auto">
            ...
            <div className="sticky top-2 z-40 mb-8 mx-2">
                {/* Master Messages Quick Alert */}
                {messages && messages.length > 0 && (
                    <div className="mb-2 bg-yellow-400 border-4 border-black p-1 overflow-hidden whitespace-nowrap rounded-md shadow-md">
                        <div className="inline-block animate-marquee text-black font-black uppercase text-sm px-4">
                            📢 {messages[messages.length - 1].text} • {messages[messages.length - 1].text} • {messages[messages.length - 1].text}
                        </div>
                    </div>
                )}
                ...
                <div className="bg-[#c6c6c6] border-4 border-[#333] p-3 flex flex-col md:flex-row items-center justify-between shadow-[0_8px_0_rgba(0,0,0,0.2)] rounded-lg">
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-auto mt-4 md:mt-0">
                        ...
                        <div className="w-full md:w-72 h-6 bg-[#333] border-4 border-white relative rounded-md overflow-hidden shadow-inner">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center text-[11px] font-black text-white drop-shadow-md uppercase">
                        ...
                        <div className="p-4 min-h-[300px]">
                            {tasks.filter(t => t.timeOfDay === section.id).length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center opacity-30 py-16">
                                    ...
                                    <span className="font-black font-pixel mt-4 text-gray-600 uppercase">Sem Tarefas</span>
                                </div>
                            ) : (
            ...
            <div className="fixed bottom-6 right-6 flex flex-col gap-4 z-50">
                ...
                >
                    <Palette size={32} />
                    <span className="text-[10px] font-black font-pixel mt-1 uppercase">Construir</span>
                </button>
                ...
                >
                    <Gift size={32} />
                    <span className="text-[10px] font-black font-pixel mt-1 uppercase">Loja</span>
                </button>
            </div>
                ...
                <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-6 backdrop-blur-sm">
                    <div className="panel-game max-w-sm w-full p-0 bg-white overflow-hidden">
                        ...
                        <div className="p-8 text-center">
                            <h3 className="font-display text-4xl text-gray-800 mb-4 uppercase">{selectedTask.title}</h3>
                            ...
                            {isUploading ? (
                                <div className="py-8 flex flex-col items-center gap-4">
                                    ...
                                    <span className="text-2xl font-black text-green-600 font-pixel uppercase">Enviando...</span>
                                </div>
                            ) : (
            ...
            {activeModal === 'SHOP' && (
                <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4">
                    <div className="panel-dark w-full max-w-4xl flex flex-col max-h-[90vh] rounded-xl overflow-hidden shadow-2xl">
                        ...
                        <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 overflow-y-auto bg-gray-700">
                            {rewards.map(r => {
                                ...
                                return (
                                    <div key={r.id} className={`p-4 rounded-lg border-4 flex flex-col items-center gap-3 transition-all ${canBuy ? 'bg-gray-800 border-yellow-500 hover:scale-105' : 'bg-gray-900 border-gray-600 opacity-50'}`}>
                                        <div className="text-5xl drop-shadow-lg" style={r.type === 'block' ? { color: r.blockColor } : {}}>{r.icon}</div>
                                        <div className="text-center">
                                            <p className="font-black text-white text-sm leading-tight uppercase">{r.title}</p>
                                        ...
                                        <button
                                            disabled={!canBuy}
                                            onClick={() => { onBuyReward(r.id); sfx.play('click'); }}
                                            className={`w-full py-2 rounded-md font-black text-xs border-b-4 transition-all active:border-b-0 ${canBuy ? 'bg-green-500 border-green-800 text-white' : 'bg-gray-700 border-gray-900 text-gray-500'}`}
                                        >
                                            COMPRAR
                                        </button>
                                    </div>
