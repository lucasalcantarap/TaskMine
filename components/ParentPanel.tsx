...
const ParentPanel: React.FC<ParentPanelProps> = ({
    ...
    onAdjustCurrency, onAddReward, onDeleteReward, onUpdateSettings, onSendMessage
}) => {
    ...
    return (
        <div className="w-full text-gray-100 max-w-6xl mx-auto">
            ...
            <div className="bg-gray-800 border-4 border-gray-700 p-6 rounded-b-xl min-h-[500px] shadow-inner">
                ...
                {/* Master Messages Quick Send */}
                <div className="mb-6 bg-zinc-900/50 p-4 border-2 border-zinc-700 rounded-lg flex gap-4">
                    <input
                        type="text"
                        id="parent-msg-input"
                        className="input-game flex-grow border-none bg-transparent text-white"
                        placeholder="Mande um incentivo para seu herói..."
                    />
                    <button
                        onClick={() => {
                            const input = document.getElementById('parent-msg-input') as HTMLInputElement;
                            if (input.value.trim()) {
                        ...
                        className="btn-game btn-primary py-2"
                    >ENVIAR</button>
                </div>
                    ...
                    <div className="max-w-4xl mx-auto space-y-12 py-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <div className="bg-gray-700 p-6 rounded-xl border-t-8 border-blue-500 shadow-xl">
                                <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center font-display text-xl text-white mb-4">1</div>
                                <h3 className="font-display text-xl text-white mb-2 uppercase">Missões</h3>
                                <p className="text-xs text-gray-300 leading-relaxed font-bold">Crie tarefas na aba <strong>"Quests"</strong>. Elas dividem o dia em períodos (Manhã/Tarde/Noite).</p>
                            </div>
                            <div className="bg-gray-700 p-6 rounded-xl border-t-8 border-yellow-500 shadow-xl">
                                <div className="w-10 h-10 bg-yellow-500 rounded-full flex items-center justify-center font-display text-xl text-white mb-4">2</div>
                                <h3 className="font-display text-xl text-white mb-2 uppercase">Mensagens</h3>
                                <p className="text-xs text-gray-300 leading-relaxed font-bold">Use o campo de <strong>Mensagem Master</strong> para enviar incentivos que aparecem no letreiro da criança.</p>
                            </div>
                            <div className="bg-gray-700 p-6 rounded-xl border-t-8 border-green-500 shadow-xl">
                                <div className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center font-display text-xl text-white mb-4">3</div>
                                <h3 className="font-display text-xl text-white mb-2 uppercase">Economia</h3>
                                <p className="text-xs text-gray-300 leading-relaxed font-bold">Aprovar fotos dá <strong>Moedas (Esmeraldas)</strong>. Elas são usadas no <strong>Builder Mode</strong> ou na Loja.</p>
                            </div>
                            <div className="bg-gray-700 p-6 rounded-xl border-t-8 border-red-500 shadow-xl">
                                <div className="w-10 h-10 bg-red-500 rounded-full flex items-center justify-center font-display text-xl text-white mb-4">4</div>
                                <h3 className="font-display text-xl text-white mb-2 uppercase">XP & Nível</h3>
                                <p className="text-xs text-gray-300 leading-relaxed font-bold">Ganhar XP sobe o <strong>Nível</strong> e desbloqueia novos <strong>Avatares</strong> épicos para o herói.</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-blue-900/30 p-6 rounded-xl border-4 border-blue-500/50">
                                <h3 className="font-display text-xl text-blue-300 mb-3 flex items-center gap-3"><Zap /> Bônus Heroico</h3>
                                <p className="text-blue-100 text-sm font-bold leading-relaxed">
                                    Tarefas do período da <strong>Noite</strong> ou com descrições detalhadas dão <strong>XP em dobro</strong>!
                                    Incentive seu herói a caprichar nas missões mais difíceis.
                                </p>
                            </div>
                            <div className="bg-orange-900/30 p-6 rounded-xl border-4 border-orange-500/50">
                                <h3 className="font-display text-xl text-orange-300 mb-3 flex items-center gap-3"><AlertCircle /> Hardcore Mode</h3>
                                <p className="text-orange-100 text-sm font-bold leading-relaxed">
                                    Cuidado! Deixar tarefas pendentes do dia anterior causa uma penalidade de <strong>-20 HP</strong>.
                                    Use o <strong>Painel Admin</strong> para curar (HP) ou dar bônus extras de Esmeraldas.
                                </p>
                            </div>
                        </div>
                    </div>
